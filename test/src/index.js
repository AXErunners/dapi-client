  ...
  'dapschema': {
    ...
    'AxePay': {
      ...
      'user': {
        ...
        'allOf': [
          {
            '$ref': 'http://axe.org/schemas/sys#/definitions/dapobjectbase'
          }
        ],
        '_isrole': true,
        'properties': {
      ...
      'contact': {
        ...
        'properties': {
          ...
          'user': {
            '$ref': 'http://axe.org/schemas/sys#/definitions/relation'
          },
          'hdextpubkey': {
            'type': 'string'
          }
  ...
  before(() => {
    ...
    sinon.stub(rpcClient, 'request')
      .callsFake(async function (url, method, params) {
          ...
          if (method === 'getUser') {
            /*
            Since axe schema uses fs, it would be impossible to run tests in browser
            with current version of validation from axe-schema
            */
  ...
  describe('constructor', () => {
    it('Should set seeds and port, if passed', async () => {
      const dapi = new DAPIClient({
        seeds: [{ service: '127.1.2.3:19937' }],
        port: 1234
      });
      expect(dapi.DAPIPort).to.be.equal(1234);
      expect(dapi.MNDiscovery.masternodeListProvider.DAPIPort).to.be.equal(1234);
      expect(dapi.MNDiscovery.masternodeListProvider.seeds).to.be.deep.equal([{ service: '127.1.2.3:19937' }]);
      expect(dapi.MNDiscovery.seeds).to.be.deep.equal([{ service: '127.1.2.3:19937' }]);
