...
describe('basicAPIs', () => {
    ...
    before(async function it() {
        ...
        transactionIdSendToAddress = await masterNode.axeCore.getApi().sendToAddress(faucetAddress, 100);
        await dapiClient.generate(20);
        let result = await masterNode.axeCore.getApi().getAddressUtxos({"addresses": ["ygPcCwVy7Fxg7ruxZzqVYdPLtvw7auHAFh"]});
        await wait(20000);
        ...
        it('should fetchDapContract', async function it() {
            ...
            for (let i = 0; i <= attempts; i++) {
                try {
                    ...
                    dapContractFromDAPI = await dapiClient.fetchDapContract(app.getDPContract().getId());
                    break;
                } catch (e) {
                    await dapiClient.generate(1);
                }
