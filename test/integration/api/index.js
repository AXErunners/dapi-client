...
const { startDapi } = require('@axerunners/dp-services-ctl');
...
describe('basicAPIs', () => {
  ...
  before(async function it() {
    ...
    transactionIdSendToAddress = await masterNode.axeCore.getApi()
      .sendToAddress(faucetAddress, 100);
    await dapiClient.generate(20);
    await masterNode.axeCore.getApi().getAddressUtxos({ addresses: ['ygPcCwVy7Fxg7ruxZzqVYdPLtvw7auHAFh'] });
    await wait(20000);
  });
  ...
  describe('Address', () => {
    it('should return correct getUTXO', async () => {
      ...
      const { result: coreOutput } = await masterNode.axeCore.getApi().getAddressUtxos(
    ...
    it('should return correct getAddressSummary', async () => {
      ...
      const { result: coreOutput } = await masterNode.axeCore.getApi().getAddressUtxos({ addresses: [faucetAddress] });
      expect(dapiOutput).to.be.deep.equal({
  ...
  describe('Block', () => {
    it('should return correct getBestBlockHash', async () => {
      ...
      const coreOutput = await masterNode.axeCore.getApi().getbestblockhash();
    ...
    it('should return correct getBestBlockHeight', async () => {
      ...
      const coreOutput = await masterNode.axeCore.getApi().getblockcount();
    ...
    it('should return correct getBlockHeaders', async () => {
      ...
      const coreOutput = await masterNode.axeCore.getApi().getblockheaders(blockHash);
  ...
  describe('Mempool', () => {
    it('should return correct getMempoolInfo output', async () => {
      ...
      const coreOutput = await masterNode.axeCore.getApi().getmempoolinfo();
      expect(dapiOutput).to.be.deep.equal(coreOutput.result);
    });
  });
  ...
  describe('All APIs', () => {
    ...
    it('should fetchContract', async () => {
      ...
      expectedContract.$schema = 'https://schema.axe.org/dpp-0-4-0/meta/contract';
