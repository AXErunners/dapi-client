...
describe('retry policy', () => {
  ...
  before(async () => {
    ...
    const dpContract = app.contract.create(entropy.generate(), {
      user: {
        properties: {
          avatarUrl: {
            type: 'string',
    ...
    insightURL = `http://127.0.0.1:${masterNode.insight.options.getApiPort()}/insight-api-axe`;

    transactionIdSendToAddress = await masterNode.axeCore.getApi()
      .sendToAddress(faucetAddress, 100);
    await dapiClient.generate(20);
    let result = await masterNode.axeCore.getApi()
